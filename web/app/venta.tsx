...
export default function VentaScreen() {
  ...
  const [category, setCategory] = useState(
    params.category && typeof params.category === 'string' ? params.category : 'Todas'
  );
  ...
  const categoryOptions = useMemo(
    () => ['Todas', 'Favoritos', ...Array.from(new Set(products.map((product) => product.category))).sort()],
    [products],
  );
  ...
  }, [products, query, category, availability, favorites]);
  ...
  const categoryCounts = useMemo(() => Object.fromEntries(
    categoryOptions.map((option) => [
      option,
      option === 'Todas' ? products.length : option === 'Favoritos' ? favorites.length : products.filter((product) => product.category === option).length,
    ])
  ), [categoryOptions, favorites, products]);
        ...
        <View style={styles.desktopRow}>
            ...
            <View style={styles.filterBlock}>
              ...
              {categoryOptions.map((c) => (
                <Pressable key={c} onPress={() => setCategory(c)} style={[styles.filterRow, category === c && styles.filterRowActive]}>
                  ...
                  <AnimatedNumber value={String(c === 'Todas' ? products.length : c === 'Favoritos' ? favorites.length : products.filter((p) => p.category === c).length)} style={styles.filterCount} />
                </Pressable>
              ))}
            </View>
          ...
          >
            ...
            {error ? (
              ...
              <EmptyState
                icon="search"
                title={loading ? 'Cargando catálogo…' : 'No encontramos ese producto'}
                description={loading ? 'Consultando productos y stock de tu organización.' : `No existen resultados para "${query}".`}
