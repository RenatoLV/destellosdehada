...
export default function HistorialScreen() {
  ...
  useEffect(() => {
    setAllSales(saleStorage.getAllSales());
    if (!user) return;

    let active = true;
    const loadRemoteSales = async () => {
      try {
        const remoteSales = await saleStorage.loadRemoteSales();
                ...
                : sale.status === 'recovering'
                  ...
                  : sale.status === 'rejected'
                    ...
                    : sale.status === 'conflict'
                      ? 'Requiere revisión'
                      : 'Pendiente';
              const statusVariant = sale.status === 'confirmed'
                ? 'success'
                : sale.status === 'rejected' || sale.status === 'conflict'
                  ? 'error'
                  : 'warning';
