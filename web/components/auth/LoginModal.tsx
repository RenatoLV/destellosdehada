...
function authErrorMessage(error: unknown) {
  ...
  const messages: Record<string, string> = {
    'auth/email-already-in-use': 'Este correo ya tiene una cuenta. Prueba iniciar sesión.',
    'auth/invalid-credential': 'El correo o la contraseña no coinciden.',
    'auth/invalid-email': 'El correo electrónico no es válido.',
    'auth/network-request-failed': 'No pudimos conectar. Revisa tu conexión e inténtalo nuevamente.',
    'auth/too-many-requests': 'Hubo demasiados intentos. Espera un momento antes de volver a probar.',
    'auth/weak-password': 'La contraseña debe tener al menos 6 caracteres.',
    'auth/user-disabled': 'Esta cuenta está deshabilitada. Escríbenos para ayudarte.',
  };
  return messages[code] || 'No pudimos completar el acceso. Inténtalo nuevamente.';
}
...
export function LoginModal() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await login(normalizedEmail, password, mode === 'register' ? name.trim() : undefined);
      toast.show({
        message: mode === 'login' ? 'Bienvenida de nuevo.' : 'Tu cuenta quedó creada.',
        type: 'success',
      });
      setPassword('');
      setConfirmPassword('');
        ...
        >
          ...
          <View style={styles.header}>
            ...
            <Text style={[styles.subtitle, { color: theme.colors.textSecondary }]}>
              {mode === 'login'
                ? 'Accede a tus favoritos y compras.'
                : 'Guarda tus productos favoritos y revisa tus pedidos.'}
            </Text>
          </View>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.submitWrap}>
              <Button
                label={mode === 'login' ? 'Entrar a mi cuenta' : 'Crear mi cuenta'}
            ...
            <Pressable
              ...
              style={({ hovered }) => [
                styles.guestBtn,
                hovered && { backgroundColor: theme.colors.surfaceSubdued },
              ]}
              accessibilityRole="button"
              accessibilityLabel="Continuar como invitada"
            >
              <Text style={[styles.guestBtnText, { color: theme.colors.primary }]}>Continuar como invitada</Text>
