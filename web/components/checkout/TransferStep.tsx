  ...
  return (
    <View style={styles.container}>
        ...
        {rows.map((row, idx) => (
          ...
          >
            <View style={{ flex: 1, paddingRight: 8 }}>
              ...
              <Text style={[theme.typography.bodyMedium, { color: theme.colors.text, marginTop: 2 }]}>{row.value}</Text>
            </View>
            <AnimatedPressable
              onPress={() => copyToClipboard(row.value, row.label, row.key)}
              haptic="light"
              style={[
                styles.copyBtn,
                {
      ...
      <View style={styles.footer}>
        <Button label="Ya transferí →" onPress={onNext} variant="primary" size="lg" icon="arrow-right" />
      </View>
    </View>
  );
}
